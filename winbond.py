...
class W25QFlash:
    ...
    def __init__(self, spi, cs):
        self.cs = cs
        self.spi = spi_device.SPIDevice(spi, cs, baudrate=40000000, extra_clocks=8)
        ...
        if self._ADR_LEN == 4:
            if not self._read_status_reg(16):  # not in 4-byte mode
                with self.spi as flash:
                    print("entering 4-byte address mode")
                    self._await()
                    self.cs.value = False
                    flash.write(b'\xB7')  # 'Enter 4-Byte Address Mode'
                    self.cs.value = True
    ...
    def identify(self):
        ...
        with self.spi as flash:
            self._await()
            self.cs.value = False
            flash.write(b'\x9F')  # 'Read JEDEC ID'
            buf = bytearray(3)
            flash.readinto(buf, write_value=0x00)  # manufacturer id, memory type id, capacity id
            print(buf)
            mf = buf[0]
            mem_type = buf[0]
            cap = buf[0] 
    ...
    def _read_status_reg(self, nr):  # Returns the value (0 or 1) in status register <nr> (S0, S1, S2, ...)
        ...
        stat = 2**bit & self.spi.readinto(1, 0xFF)[0]
        self.cs.value = True
        return stat
    ...
    def _await(self):  # Waits for device not to be busy and returns if so
        ...
        #self.cs.value = False
        #with self.spi as flash:
            #flash.write(b'\x05')  # 'Read Status Register-1'
            #flash.readinto(self.single_byte, write_value=0xFF)
            #print(self.single_byte[0])
            
            #print(flash.readinto(self.single_byte, write_value=0xFF)[0])
            #while 0x1 & flash.readinto(self.single_byte, write_value=0xFF)[0]:  # last bit (1) is BUSY bit in stat. reg. byte (0 = not busy, 1 = busy)
                #pass
            #self.cs.value = True
