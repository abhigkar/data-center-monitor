...
class MQ():
    ...
    RO_CLEAN_AIR_FACTOR          = 3.7      # RO_CLEAR_AIR_FACTOR=(Sensor resistance in clean air)/RO,
    ...
    def __init__(self, Ro=10, analogPin=board.A0):
        self.Ro = Ro
        self.adc = analogio.AnalogIn(analogPin)
        
        self.NH4Curve = [1,0.415,-0.393]    # two points are taken from the curve. 
                                            # with these two points, a line is formed which is "approximately equivalent"
                                            # to the original curve. 
                                            # data format:{ x, y, slope}; point1: (lg200, 0.21), point2: (lg10000, -0.59) 
        self.COCurve = [1,0.462,-0.22]     # two points are taken from the curve. 
                                            # with these two points, a line is formed which is "approximately equivalent" 
                                            # to the original curve.
                                            # data format:[ x, y, slope]; point1: (lg200, 0.72), point2: (lg10000,  0.15)
        self.H2SCurve = [1,0.222,-0.306]     # two points are taken from the curve. 
                                            # with these two points, a line is formed which is "approximately equivalent" 
                                            # to the original curve.
    ...
    def MQPercentage(self):
        val = {}
        read = self.MQRead()
        print("MQread is done...\n")
        val["NH4"]  = self.MQGetGasPercentage(read/self.Ro, self.GAS_NH4)
        val["CO"]   = self.MQGetGasPercentage(read/self.Ro, self.GAS_CO)
        val["H2S"]  = self.MQGetGasPercentage(read/self.Ro, self.GAS_H2S)
        return val
    ...
    def MQCalibration(self):
        ...
        for i in range(self.CALIBARAION_SAMPLE_TIMES):          # take multiple samples
            val += self.MQResistanceCalculation(self.adc.value)
    ...
    def MQRead(self):
        ...
        for i in range(self.READ_SAMPLE_TIMES):
            rs += self.MQResistanceCalculation(self.adc.value)
